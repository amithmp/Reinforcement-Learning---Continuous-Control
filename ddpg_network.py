...
class Actor(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, fc1_units=400,fc2_units=200):
        ...
        self.normalizer_1 = nn.BatchNorm1d(fc1_units)
    ...
    def forward(self, state):
        ...
        x = self.normalizer_1(self.fc1(state))
        x = F.relu(x)
        x = F.relu(self.fc2(x))
...
class Critic(nn.Module):
    ...
    def __init__(self, state_size, action_size, seed, fcs1_units=400, fc2_units=200):
        ...
        self.normalizer_1 = nn.BatchNorm1d(fcs1_units)
        self.normalizer_2 = nn.BatchNorm1d(fc2_units)
        self.reset_parameters()
    ...
    def forward(self, state, action):
        ...
        if state.dim() == 1:
            state = torch.unsqueeze(state,0)
        xs = self.normalizer_1(self.fcs1(state))
        xs = F.relu(xs)
